...
class PackedPhoneNumber {
  public:
    ...
    bool operator==(const PackedPhoneNumber other) const { return this->_data == other._data; }

    bool operator!=(const PackedPhoneNumber other) const { return !(*this == other); }
     ...
     * - >0 (if a > b)
     */
    int64_t compare_fast(PackedPhoneNumber other) const { return other._data - this->_data; }

    /// Returns the number's country code
    google::protobuf::uint32 country_code() const { return get_masked(_data, country_code_bits, country_code_offset); }
    ...
    uint64_t national_number() const { return get_masked(_data, national_number_bits, national_number_offset); }
    ...
    uint64_t leading_zeros() const { return get_masked(_data, leading_zeros_bits, leading_zeros_offset); }

    /// Sets the number of leading zeros
    void leading_zeros(uint64_t value) { _data = set_masked(_data, value, leading_zeros_bits, leading_zeros_offset); }
...
/*
 * If the size of the PackedPhoneNumber class changes for any reason, it will
 * trip this assertion and remind us to update the SQL definition of the
 * short_phone_number type.
 */
